(ns bolt.main
  "Kotodama WASM entrypoint for the Bolt clean-room actor (L4) — Clojure port.
  ...
  facts (`bolt.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Driver"   :plural "drivers"   :id-prefix "bolt_dri"
   ...
   {:entity "Rating"    :plural "ratings"    :id-prefix "bolt_rat"
...
(defn emit-facts
  "EAVT facts for one record: {\"bolt.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bolt-compat" :tier tier :entities entities} 200])
